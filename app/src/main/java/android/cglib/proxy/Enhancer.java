...
import android.cglib.dx.Code;
import android.cglib.dx.Comparison;
import android.cglib.dx.DexMaker;
import android.cglib.dx.FieldId;
import android.cglib.dx.Label;
import android.cglib.dx.Local;
import android.cglib.dx.MethodId;
import android.cglib.dx.TypeId;
...
public class Enhancer {
	...
	public Class<?> create() {
		...
		String subClsName = superClsName + Const.SUBCLASS_SUFFIX+"_"+Integer.toHexString(random.nextInt(Integer.MAX_VALUE));
...
//		System.out.println("[Enhancer::create()] Create class extends from \"" + superclass.getName() + "\" stored in " + cacheDir);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
 	private <S> void generateFieldsAndMethods(DexMaker dexMaker, TypeId<?> superType, TypeId<S> subType) {
		...
		dexMaker.declare(fieldId, Modifier.PRIVATE|Modifier.STATIC, null);
		...
		code.sput(fieldId, code.getParameter(0, methodInterceptorType));
		code.returnVoid();
	...
	private void hookMethod(DexMaker dexMaker, TypeId<?> superType, TypeId<?> subType, Method method, String methodName, FieldId<?, MethodInterceptor> fieldId) {
		...
		Local thisLocal = code.getThis(subType);
		code.sget(fieldId, methodInterceptorLocal);
